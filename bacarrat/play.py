...
class FriendPattern:
    def __init__(self, name: str, pattern_type: str):
        ...
        # to initialize in record_hand()

    def next_bet_choice(self) -> str:
        p = self.pattern_type
        # Patterns awaiting first non-tie
    ...
    def next_bet_amount(self, unit: float) -> float:
        ...
        idx = min(self.step, len(multipliers) - 1)
    ...
    def record_hand(self, outcome: str, unit: float):
        ...
        # Skip miss/progression on first real bet
        if self.first_bet:
        ...
        # Star 2.0 progression resets for ANY two consecutive wins
        if hit:
            self.total_hits += 1
            self.win_streak += 1
