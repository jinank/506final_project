...
class FriendPattern:
    def __init__(self, name: str, pattern_type: str):
        ...
        # First-hand flag: skip miss count on first
        ...
        if pattern_type == 'alternator_start_banker':
            ...
            # wait for first free hand, then build pattern
            self.sequence = None
            self.seq_index = 0
    ...
    def next_bet_choice(self) -> str:
        # Terrific Twos: free hand until first non-tie, then fixed pattern
    ...
    def record_hand(self, outcome: str, unit: float):
        ...
        if self.pattern_type == 'terrific_twos':
            if self.free_outcome is None and outcome in ['B','P']:
                ...
                # 10-step pattern: base twice, then alternations
