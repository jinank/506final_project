...
class FriendPattern:
    def __init__(self, name: str, pattern_type: str):
        ...
        if p == 'alternator_start_banker':
            ...
            self.sequence = ['P', 'B']
        # terrific_twos, chop, three_pattern, one_two_one, two_three_two, pattern_1313, follow_last
        # will initialize in record_hand()

    def next_bet_choice(self) -> str:
        p = self.pattern_type
        # Patterns waiting for first non-tie
    ...
    def next_bet_amount(self, unit: float) -> float:
        ...
        idx = max(0, min(self.step, len(multipliers)-1))
        return unit * multipliers[idx]
    ...
    def record_hand(self, outcome: str, unit: float):
        ...
        if p == 'pattern_1313' and self.free_outcome is None and outcome in ('B','P'):
            base, alt = outcome, ('P' if outcome=='B' else 'B')
            # 1-3-1-3 pattern: alt, alt, alt, base, repeat...
        ...
        if pred == '':
            # free hand: advance sequence pointer if any
            if self.sequence:
        ...
        # First real bet: record hit/miss but skip progression
        if self.first_bet:
        ...
        # Star 2.0 progression: reset on two consecutive wins for all
        if hit:
            self.total_hits += 1
            self.win_streak += 1
...
with st.sidebar:
    ...
    if len(top_friends) > 1 and len(bets) > 1:
        ...
        display_amount = int(max(f.next_bet_amount(session.unit) for f in top_friends))
    st.markdown(f"""
