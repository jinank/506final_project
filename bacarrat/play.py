...
class FriendPattern:
    ...
    def record_hand(self, outcome: str, unit: float):
        ...
        # First real bet: count first miss as step 1
        if self.first_bet:
...
class Session:
    ...
    def reset(self):
        types = ['banker_only','player_only','alternator_start_banker','alternator_start_player',
...
with st.sidebar:
    ...
    # Conservative start suggestion for >10 misses
    ...
    if cons:
        ...
        st.markdown(f"<div style='background-color:lightgreen; padding:8px; border-radius:4px;'>"
                    f"Conservative Start: Bet {session.unit}X unit on {names}</div>",
