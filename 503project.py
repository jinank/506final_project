...
temps = st.sidebar.selectbox("Temperature levels", options=[(0.3,0.5,0.8)], format_func=lambda x: f"{x[0]} / {x[1]}")
topp_levels = st.sidebar.selectbox("Top-p levels", options=[(0.2,0.9)], format_func=lambda x: f"{x[0]} / {x[1]}")
